...
const getProtocolData =
    name => ({
        mysql: {
            NO_SUCH_TABLE_ERR: 'ER_NO_SUCH_TABLE',
            DROP_DATABASE_SQL: 'DROP SCHEMA IF EXISTS lego',
        }
    }[name] || {
        NO_SUCH_TABLE_ERR: '42P01',
        DROP_DATABASE_SQL: 'DROP SCHEMA IF EXISTS lego CASCADE',
    });
...
describe('migrations', function () {
    ...
    const {NO_SUCH_TABLE_ERR, DROP_DATABASE_SQL} = getProtocolData(PROTOCOL);
    ...
    afterEach(function () {
        sandbox.restore();
        return Lego.sql`${Lego.raw(DROP_DATABASE_SQL)}`;
    });
    ...
    describe('going up', function () {
        ...
        it('empty to 1 to empty', function () {
            return Migrations.migrate(0, 1)
                ...
                .then(assert.fail)
                .catch(function (error) {
                    assert.equal(error.code, NO_SUCH_TABLE_ERR);
                });
        });
